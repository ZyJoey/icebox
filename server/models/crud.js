...
var server = new Server('localhost', config.mongodb.port, { auto_reconnect: true }, { safe: true }),
...
exports.createFood = function (req, res) {
  ...
  function updateFood() {
    db.collection('list').update({ "username": username }, {
      $push: {
        "foodList": {
          "id": autoindex,
          "name": req.body.name,
...
exports.updateFood = function (req, res) {
  var username = req.session.username,
    ...
    data = {};
  db.collection("list").update({ "username": username, "foodList.id": id }, {
    $set: {
